...
const CurrentEditionPeopleSection: React.FC = () => {
  ...
  return (
    <motion.section
      ...
      className="w-full relative flex flex-col items-center justify-center md:py-20"
    >
      <motion.h2
      ...
      <motion.div className="flex flex-col gap-6 py-10 md:w-[70vw] items-center">
        {isInView
          ? CARDS.map((card, index) => (
              ...
              >
                <h3 className="font-[900] italic text-[40px] w-full md:w-[30%] text-center md:self-start">
                  {card.title}
                </h3>
                <div className="text-xl text-black flex flex-col justify-between h-full gap-8 md:gap-0">
      ...
      <motion.div className="flex justify-center items-center mb-10 max-w-[60vw] w-full gap-6">
        <motion.div className="relative md:w-[30%] aspect-[4/3]">
          <Image
        ...
        <motion.ul className="md:w-[50%] md:max-h-39 font-work-sans text-sm grid grid-cols-1 md:grid-cols-2">
          {ACTIONS.map((action, index) => (
            <motion.li
              key={index}
              className="mb-6 font-bold text-md flex items-start gap-2"
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
      ...
      <motion.div
        className="bg-orange text-white px-10 py-10 md:py-6 rounded-3xl w-full max-w-[1200px] md:h-[260px] flex flex-col md:flex-row justify-start items-center gap-14 font-montserrat"
        initial={{ y: 20, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
      ...
      <figure className="absolute bottom-0 left-64 transform translate-y-4 hidden md:block w-[170px] h-[170px] rounded-[40px] -rotate-[66deg] bg-orange/20" />
    </motion.section>
  );
};
